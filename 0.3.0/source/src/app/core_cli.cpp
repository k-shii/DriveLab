...
int runDryRunBootstrap(std::ostream& output, std::ostream& error_output) {
    ...
    output << "DriveLab Core " << kVersion << '\n'
           << "Mode: dry-run\n"
           << "Inventory source: mock fixtures (native inventory is not wired to dry-run)\n"
...
int runApplicationCli(const std::vector<std::string>& arguments,
                      ...
                      DemoUiRunner run_production_ui) {
    ...
    if (run_production_ui) return run_production_ui();
    error_output << "Production UI is not available in this build.\n";
    return 1;
}
