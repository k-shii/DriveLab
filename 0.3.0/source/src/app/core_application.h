...
public:
    ...
    static Result<std::unique_ptr<CoreApplication>> createProduction(
        AppConfig config, ILogger& logger,
        std::unique_ptr<ProductionInventorySource> source);
    Result<void> startProductionScan();
    void waitForProductionScan();
    void cancelProductionScan();
    Result<void> rescanProduction();
    [[nodiscard]] Result<std::shared_ptr<const ProductionSnapshot>> productionSnapshotView() const;
    [[nodiscard]] Result<ProductionSnapshot> productionSnapshot() const;
...
private:
    ...
    std::unique_ptr<ProductionInventorySource> production_source_;
    void publishProduction(std::shared_ptr<const ProductionSnapshot> snapshot);
    mutable std::mutex production_snapshot_mutex_;
    std::mutex production_control_mutex_;
    std::shared_ptr<const ProductionSnapshot> production_snapshot_ = std::make_shared<const ProductionSnapshot>();
    std::atomic_bool production_running_{false};
    // Last member: stop and join before providers or synchronization state die.
    std::jthread production_worker_;
};
