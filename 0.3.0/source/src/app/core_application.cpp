...
CoreApplication::CoreApplication(AppConfig config, ILogger& logger)
    : config_(std::move(config)),
      logger_(logger),
      inventory_(config_.mode == ExecutionMode::Real
      ...
      report_writer_("Report file writes are unavailable in this application") {
    if (config_.mode == ExecutionMode::Real) {
        capabilities_.set({CapabilityId::Inventory, CapabilityAvailability::Available,
                           "production", "Read-only discovery and safety assessment"});
        for (const auto id : {CapabilityId::Health, CapabilityId::AtaInspection,
                              CapabilityId::Benchmark, CapabilityId::Sanitize,
                              CapabilityId::Latency, CapabilityId::Reporting}) {
            capabilities_.set({id, CapabilityAvailability::NotImplemented,
                               "none", "Unavailable in discovery and safety mode"});
        }
        capabilities_.set({CapabilityId::ProcessExecution,
                           CapabilityAvailability::DisabledByMode,
                           "disabled", "Production discovery executes no external processes"});
        return;
    }
...
Result<CoreSnapshot> CoreApplication::snapshot() {
    if (config_.mode == ExecutionMode::Real) {
        return Result<CoreSnapshot>::failure({ErrorCode::Unavailable, "CoreApplication",
            "Use productionSnapshot for resolved production identity and classification"});
    }
...
Result<JobId> CoreApplication::submitHealth(const Drive& drive, HealthTestType test) {
    if (config_.mode == ExecutionMode::Real)
        return Result<JobId>::failure({ErrorCode::Unavailable, "CoreApplication", "Health is not implemented in production"});
...
Result<JobId> CoreApplication::submitAtaInspection(
    ...
    if (config_.mode == ExecutionMode::Real)
        return Result<JobId>::failure({ErrorCode::Unavailable, "CoreApplication", "ATA inspection is not implemented in production"});
...
Result<JobId> CoreApplication::submitBenchmark(
    ...
    if (config_.mode == ExecutionMode::Real)
        return Result<JobId>::failure({ErrorCode::Unavailable, "CoreApplication", "Benchmarks is not implemented in production"});
...
Result<JobId> CoreApplication::submitSanitize(const Drive& drive, SanitizeMethod method) {
    if (config_.mode == ExecutionMode::Real)
        return Result<JobId>::failure({ErrorCode::Unavailable, "CoreApplication", "Sanitization is not implemented in production"});
