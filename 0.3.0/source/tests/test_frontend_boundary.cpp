...
int main() {
    return drivelab::test::run([] {
        ...
        const std::vector<std::string> forbidden = {
            ...
            "<signal.h>",
            "platform/linux/",
            "ProductionInventorySource",
            "LinuxOwnershipSource"
        };
        const std::regex storage_command{
            R"(["'](lsblk|smartctl|fio|hdparm|nwipe|ioping)([[:space:]"']))"
        };

        const auto ui = std::filesystem::path{DRIVELAB_SOURCE_DIR} / "src" / "ui";
        for (const auto& frontend : {prototype, ui}) {
        for (const std::filesystem::directory_entry& entry :
             std::filesystem::recursive_directory_iterator(frontend)) {
